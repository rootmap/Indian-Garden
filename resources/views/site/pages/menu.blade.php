          ...
          <div class="col-md-4 menu_side_bar">
              ...
              @if(isset($category))
                ...
                @foreach($category as $c)
                   <li class="">
                      <a href="javascript:void(0);" data-group="{{ str_replace(' ','',$c->name) }}" class="{{$i==0?'active':''}} menuitemLoad">
                        {{$c->name}}
                      </a>
                    </li>
                    <?php $i++; ?>
                          ...
                          @if(count($m['scat'])>0)
                              ...
                              @foreach($m['scat'] as $r)
                                  @if($M==0)
                                    <div class="row">
                                  @endif
                                  <div class="col-md-6 item-box mb-1 all-item {{ str_replace(' ','',$m['name']) }}">
                                      <h4 class="item-title">
                                          <span>{{ $r['name'] }}</span>
                                      </h4>
          ...
          <div class="col-md-12">
                  ...
                  @if(isset($MenuItem))
                      @foreach($MenuItem as $m)
                          ...
                          <div class="row">
                            <div class="col-md-12">
                                <h3 data-heading="{{$m['name']}}" data-cat="intcat{{$m['id']}}" class="mob-cat-menu"><i class="fa fa-caret-right"></i> {{$m['name']}} <span>&nbsp;</span></h3>
                            </div>
                          </div> 
                          ...
                          @if(count($m['scat'])>0)
                              ...
                              @foreach($m['scat'] as $r)
                                  ...
                                  @endif
                                  <div class="col-md-6 item-box mb-1 {{'intcat'.$m['id']}} all-item {{ str_replace(' ','',$m['name']) }}">
                                      <h4 class="item-title">
                                          <span>{{ $r['name'] }}</span>
                                      </h4>
                                      <span class="menu-item-price uppercase">{{ $r['price'] }}</span>
